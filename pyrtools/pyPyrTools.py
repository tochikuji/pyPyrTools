...
class Spyr(pyramid):
    ...
    def spyrLev(self, lev):
        if lev < 0 or lev > self.spyrHt() - 1:
            print('Error: level parameter must be between 0 and %d!' %
                  (self.spyrHt() - 1))
            return
    ...
    def spyrBand(self, lev, band):
        ...
        if band < 0 or band > self.numBands() - 1:
            print('Error: band parameter must be between 0 and %d!' %
                  (self.numBands() - 1))
    ...
    def reconPyr(self, *args):
        ...
        if levs == 'all':
            ...
            if (levs < 0).any() or (levs >= maxLev).any():
                print(
                    "Error: level numbers must be in the range [0, %d]." % (maxLev - 1))
                return
            else:
                levs = numpy.array(levs)
        ...
        if bands == 'all':
            ...
            if (bands < 0).any() or (bands > bfilts.shape[1]).any():
                print("Error: band numbers must be in the range [0, %d]." % (
                    self.numBands() - 1))
                return
            else:
                bands = numpy.array(bands)
    ...
    def showPyr(self, prange='auto2', gap=1, scale=2, disp='qt'):
        ...
        if prange == 'auto1':
            ...
            for lnum in range(1, ht + 1):
                for bnum in range(nbands):
                    band = self.band(pyPyrUtils.LB2idx(
                        lnum, bnum, ht + 2, nbands))
        ...
        for lnum in range(1, ht + 1):
            ...
            if nbands < 5:         # to align edges
                sz += gap * (ht - lnum)
            llpos[ind1:ind1 + nbands, :] = numpy.dot(
                relpos, numpy.diag(sz)) + (numpy.ones((nbands, 1)) * basepos)
        ...
        llpos = llpos - \
            ((numpy.ones((nind, 2)) * numpy.amin(llpos, axis=0)) + 1) + 1
...
class SFpyr(Spyr):
    ...
    def __init__(self, *args):    # (image, height, order, twidth)
        ...
        (Xrcos, Yrcos) = pyPyrUtils.rcosFn(
            twidth, (-twidth / 2.0), numpy.array([0, 1]))
        ...
        for i in range(ht):
            ...
            for b in range(nbands):
                ...
                banddft = ((-numpy.power(-1 + 0j, 0.5))**order) * \
...
class Lpyr(pyramid):
    ...
    def showPyr(self, pRange=None, gap=1, scale=None, disp='qt'):
        ...
        if pRange == 'auto1':
            ...
            pRange[nind - 1,
                   :] = numpy.array([av - 2 * stdev, av + 2 * stdev]);
        elif pRange == 'indep2':
            pRange = numpy.zeros(nind, 2)
            for bnum in range(0, nind - 1):
                band = self.band(bnum)
        ...
        if oned == 1:
            ...
            ctr = numpy.array(
                [self.band(0).shape[0] + 1 + gap, 1]).astype(float)
            ...
            for bnum in range(nind):
                ...
                dirr = numpy.dot(dirr, numpy.array(
                    [[0, -1], [1, 0]]))  # ccw rotation
...
class Wpyr(Lpyr):
        ...
        for lev in allLevs:
            ...
            if lev == 0:
                ...
                if (len(self.pyrSize[0]) == 1 or self.pyrSize[0][0] == 1 or
                    ...
                    hres_sz = numpy.array(
                        [self.pyrSize[resIdx - 1][0], res_sz[1]])
                imageIn = res.copy()
                if res_sz[0] == 1:
    ...
    def showPyr(self, prange=None, gap=1, scale=None, disp='qt'):
        ...
        if nbands == 1:   # 1D signal
            ...
            # make position list positive, and allocate appropriate image:
            llpos = llpos - \
                ((numpy.ones((nind, 1)) * numpy.amin(llpos, axis=0)) + 1) + 1
            urpos = llpos + self.pyrSize
