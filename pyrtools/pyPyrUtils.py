...
def namedFilter(name):
    if len(name) > 5 and name[:5] == "binom":
        ...
        kernel = numpy.array(
            [[-0.076103], [0.3535534], [0.8593118], [0.3535534], [-0.076103]])
    elif name is "qmf9":
        kernel = numpy.array([[0.02807382], [-0.060944743], [-0.073386624], [0.41472545], [
                             0.7973934], [0.41472545], [-0.073386624], [-0.060944743], [0.02807382]])
    elif name is "qmf13":
        kernel = numpy.array([[-0.014556438], [0.021651438], [0.039045125], [-0.09800052], [-0.057827797], [0.42995453], [
                             0.7737113], [0.42995453], [-0.057827797], [-0.09800052], [0.039045125], [0.021651438], [-0.014556438]])
    elif name is "qmf8":
        kernel = math.sqrt(2) * numpy.array([[0.00938715], [-0.07065183], [0.06942827], [
            0.4899808], [0.4899808], [0.06942827], [-0.07065183], [0.00938715]])
    elif name is "qmf12":
        kernel = math.sqrt(2) * numpy.array([[-0.003809699], [0.01885659], [-0.002710326], [-0.08469594], [0.08846992], [
            0.4843894], [0.4843894], [0.08846992], [-0.08469594], [-0.002710326], [0.01885659], [-0.003809699]])
    elif name is "qmf16":
        kernel = math.sqrt(2) * numpy.array([[0.001050167], [-0.005054526], [-0.002589756], [0.0276414], [-0.009666376], [-0.09039223], [0.09779817], [
            0.4810284], [0.4810284], [0.09779817], [-0.09039223], [-0.009666376], [0.0276414], [-0.002589756], [-0.005054526], [0.001050167]])
    elif name is "haar":
        kernel = numpy.array([[1], [1]]) / math.sqrt(2)
    elif name is "daub2":
        kernel = numpy.array([[0.482962913145], [0.836516303738], [
                             0.224143868042], [-0.129409522551]])
    elif name is "daub3":
        kernel = numpy.array([[0.332670552950], [0.806891509311], [
                             0.459877502118], [-0.135011020010], [-0.085441273882], [0.035226291882]])
    elif name is "daub4":
        kernel = numpy.array([[0.230377813309], [0.714846570553], [0.630880767930], [-0.027983769417],
                              [-0.187034811719], [0.030841381836], [0.032883011667], [-0.010597401785]])
    elif name is "gauss5":  # for backward-compatibility
        kernel = math.sqrt(
            2) * numpy.array([[0.0625], [0.25], [0.375], [0.25], [0.0625]])
        ...
        for j in range(recon2.shape[1]):
            if numpy.absolute(recon1[i, j].real - recon2[i, j].real) > math.pow(10, -11):
                print("real: i=%d j=%d %.15f %.15f diff=%.15f" % (
                    i, j, recon1[i, j].real, recon2[i, j].real, numpy.absolute(recon1[i, j].real - recon2[i, j].real)))
                return 0
            ...
            if isinstance(recon1, complex):
                if int(math.log(numpy.abs(recon1[i, j].imag), 10)) > 1:
                    prec = prec + \
                        int(math.log(numpy.abs(recon1[i, j].imag), 10))
                    if prec > 0:
                        prec = -1
                print(prec)
                if numpy.absolute(recon1[i, j].imag - recon2[i, j].imag) > math.pow(10, prec):
                    print("imag: i=%d j=%d %.15f %.15f diff=%.15f" % (
                        i, j, recon1[i, j].imag, recon2[i, j].imag, numpy.absolute(recon1[i, j].imag - recon2[i, j].imag)))
                    return 0
                    ...
                    if matTmp[i, j] != pyPyr.pyr[idx][i, j]:
                        if (math.fabs(matTmp[i, j] - pyPyr.pyr[idx][i, j]) >
                            ...
                            print("failed level:%d element:%d %d value:%.15f %.15f" % (
                                idx, i, j, matTmp[i, j], pyPyr.pyr[idx][i, j]))
                            return 0
    ...
    if len(args) > 2:
        if isinstance(args[2], str):
            if args[2] == 'even' or args[2] == 'EVEN':
                ...
                print(
                    "Error: only 'even' and 'odd' are valid entries for the third input parameter.")
                return
        else:
...
def var2(*args):
    ...
    if(numpy.isreal(args[0]).all()):
        res = sum(sum((args[0] - mn)**2)) / \
            max(numpy.prod(args[0].shape) - 1, 1)
    else:
...
def imStats(*args):
    ...
    if len(args) == 2:
        ...
        print('  Mean: %f,  Stdev (rmse): %f,  SNR (dB): %f' %
              (mean, numpy.sqrt(v), snr))
    else:
        (mn, mx) = range2(args[0])
        mean = args[0].mean()
        var = var2(args[0])
...
def showIm(*args):
    ...
    incr = int(numpy.ceil(
        float(matrix.max() - matrix.min() + 1) / float(nshades)))
...
def histo(*args):
    ...
    if len(args) > 1:
        if args[1] < 0:
            ...
            if tmpNbins != args[1]:
                print('Warning: Using %d bins instead of requested number (%d)' % (
                    tmpNbins, args[1]))
    else:
...
def imGradient(*args):
    ...
    gp = numpy.array([0.037659, 0.249153, 0.426375,
                      0.249153, 0.037659]).reshape(5, 1)
    gd = numpy.array([-0.109604, -0.276691, 0.000000,
                      0.276691, 0.109604]).reshape(5, 1)
