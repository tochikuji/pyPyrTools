...
class progressbar:
    ...
    def update(self, percentage, minsecs=None):
        ...
        if minsecs is None or now - self.last_update > minsecs or (self.last_perc < 100 and percentage == 100):
            display(Javascript("$('div#{}').width('{}%')".format(
                self.divid, int(percentage))))
...
def nbimageLCV(dlist, vmin=None, vmax=None, vsym=False, saveas=None,
    ...
    for data in dlist:
        ...
        label = 'Range: [%.1f, %.1f]<br>Dims: [%d, %d]*%.2f' % (
            vmin, vmax, data.shape[0], data.shape[1], currZoom)
        #s += """<th><center><img src="%s"/>%s</center></th>"""%(im,label)
        # if title != "":
        #    title += "<br>"
        s += """<th><center>%s<img src="%s"/>%s</center></th>""" % (
            title, im, label)
        displayColCtr += 1
...
def nbvideo(data, vmin=None, vmax=None, vsym=False, fps=1, loop=False, theora=8, h264=18, vp8=None, saveas=None, counter=None, counter_color=255):
    ...
    if counter is not None:
        data = asarray([draw_text(d, str(i), color=counter_color,
                                  pos=counter) for i, d in enumerate(data)])

    cmdstring = ('ffmpeg',
                 '-f', 'rawvideo',
                 '-r', '{}'.format(int(fps)),
                 '-s', '{}x{}'.format(data.shape[2], data.shape[1]),
                 '-pix_fmt', 'gray' if data.ndim == 3 else {
                     3: 'rgb24', 4: 'rgba'}[data.shape[-1]],
                 '-i', '-',
                 )
    data = data.tostring()
    ...
    if theora is not None:
        ...
        p = Popen(cmdstring + cmdstring_add,
                  stdin=PIPE, stdout=PIPE, shell=False)
        s = p.communicate(data)[0]
        if saveas is not None:
            open(saveas + '.ogg', 'wb').write(s)
        html += '<source src="data:video/ogg;base64,{}" type="video/ogg" />'.format(
            s.encode('base64'))
    ...
    if vp8 is not None:
        ...
        html += '<source src="data:video/webm;base64,{}" type="video/webm" />'.format(
            s.encode('base64'))
...
def dimwrap(data, axis=0, aspect=1, fill=0):
    ...
    for x in range(1, data.shape[axis] + 1):
        ...
        if error <= best_cost:
            best_cost = error
            best_shape = int(y), int(x)
    flatshape = data.shape[:axis] + \
        (best_shape[0] * best_shape[1], ) + data.shape[(axis + 1):]
